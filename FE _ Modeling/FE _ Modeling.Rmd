...
load_reqs(c("tidyr", "dplyr", "data.table", "purrr", "ggplot2", "corrplot", 
            "gridExtra", "grid", "cowplot", "caret", "MLmetrics", "DMwR",
            "ROSE", "ranger"))
```
...
#### Reducing the levels for variables days and month
As seen earlier, days and month are considered as factor variables. However, this means that there are 30+11 dummy variables in the model. We try our next model iterations by reducing days to weeks (1-7=week1, 7-14=week2, etc.) and months to quarters. 

```{r}
train_preproc$day_bin <- cut(as.numeric(train_preproc$day), 
...
train_preproc$month_bin <- factor(car::recode(train_preproc$month, "c('jan','feb','mar') = 'Q1';
                                        c('apr', 'may', 'jun')= 'Q2';
                                        c('jul', 'aug', 'sep')= 'Q3'; 
                                        c('oct', 'nov', 'dec')= 'Q4'"))

train_preproc$month <- NULL
...
```{r}
# update pipeline
summ2 <- rbind(summ2, log_finalfe1$results[1:3])
summ2$parameter[nrow(summ2)] <- "random binning"
summ2
```
...
tuneGrid <- data.table(expand.grid(mtry=c(5,15), splitrule = "gini",
...
log_rf <- train(label~.,
                 data = df_train_preproc,
                 method = 'ranger',
                 num.trees = 50, tuneGrid = tuneGrid,
...
# Trial - Random Forest on entire train dataset

```{r}
set.seed(7)
train_rf <- train(label~.,
                 data = train_preproc,
                 method = 'ranger',
                 num.trees = 50, tuneGrid = tuneGrid,
                 trControl = ctrl, metric = "recall"
...
Since random forest does not seem to improve model performance to a decent level, we try other methods. 
